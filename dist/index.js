...
var users = mysqlTable("users", {
  ...
  // Links both users in a couple
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
  lastSignedIn: timestamp("lastSignedIn").defaultNow().notNull()
});
var couples = mysqlTable("couples", {
  id: int("id").autoincrement().primaryKey(),
  ...
  targetLocation: varchar("targetLocation", { length: 255 }).default("Sant Feliu de Gu\xEDxols"),
  currentSavings: decimal("currentSavings", { precision: 12, scale: 2 }).default("0"),
  happinessLevel: int("happinessLevel").default(50),
  // 0-100 scale
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull()
});
var monthlyIncomes = mysqlTable("monthlyIncomes", {
  id: int("id").autoincrement().primaryKey(),
...
var workSchedules = mysqlTable("workSchedules", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull(),
...
var shoppingListItems = mysqlTable("shoppingListItems", {
  id: int("id").autoincrement().primaryKey(),
  coupleId: int("coupleId").notNull(),
...
var receipts = mysqlTable("receipts", {
  ...
  date: date("date").notNull(),
  items: json("items"),
  // JSON array of items with prices
...
async function getOrCreateCouple(user1Id, user2Id) {
  const db = await getDb();
...
async function getCoupleByUserId(userId) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
async function getMonthlyExpenses(coupleId, month) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
async function getTotalSavings(coupleId) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
var appRouter = router({
  ...
  // Couple management
  couple: router({
    getOrCreate: protectedProcedure.input(z2.object({ partnerId: z2.number().optional() })).mutation(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db) throw new Error("Database not available");
    ...
    updateHappiness: protectedProcedure.input(z2.object({ level: z2.number().min(0).max(100) })).mutation(async ({ ctx, input }) => {
      const couple = await getCoupleByUserId(ctx.user.id);
  ...
  finances: router({
    addExpense: protectedProcedure.input(z2.object({
      category: z2.string(),
      amount: z2.number(),
    ...
    getMonthlyExpenses: protectedProcedure.input(z2.object({ month: z2.date() })).query(async ({ ctx, input }) => {
      const couple = await getCoupleByUserId(ctx.user.id);
      if (!couple) throw new Error("Couple not found");
      const expenses2 = await getMonthlyExpenses(couple.id, input.month);
      return expenses2;
    }),
    getTotalSavings: protectedProcedure.query(async ({ ctx }) => {
      ...
      return { savings: Number(savings) || 0, target: 15e4 };
    })
  }),
  // Shopping list
  shoppingList: router({
    ...
    toggleItem: protectedProcedure.input(z2.object({ itemId: z2.number(), completed: z2.boolean() })).mutation(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db) throw new Error("Database not available");
      await db.update(shoppingListItems).set({ completed: input.completed }).where(eq2(shoppingListItems.id, input.itemId));
      return { success: true };
    })
  }),
  // Daily tasks
  tasks: router({
    getTasks: protectedProcedure.query(async ({ ctx }) => {
      const couple = await getCoupleByUserId(ctx.user.id);
      if (!couple) throw new Error("Couple not found");
      return await getDailyTasks(couple.id, /* @__PURE__ */ new Date());
    }),
    ...
    completeTask: protectedProcedure.input(z2.object({ taskId: z2.number(), completed: z2.boolean() })).mutation(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db) throw new Error("Database not available");
      await db.update(dailyTasks).set({ completed: input.completed }).where(eq2(dailyTasks.id, input.taskId));
      return { success: true };
    })
  }),
  // Recipes
  recipes: router({
    ...
    addRecipe: protectedProcedure.input(z2.object({
      title: z2.string(),
      description: z2.string().optional(),
      ingredients: z2.array(z2.string()).optional(),
      instructions: z2.string().optional(),
      prepTime: z2.number().optional(),
      cookTime: z2.number().optional(),
      servings: z2.number().optional()
    })).mutation(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db) throw new Error("Database not available");
      const couple = await getCoupleByUserId(ctx.user.id);
    ...
    toggleFavorite: protectedProcedure.input(z2.object({ recipeId: z2.number(), isFavorite: z2.boolean() })).mutation(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db) throw new Error("Database not available");
      await db.update(recipes).set({ isFavorite: input.isFavorite }).where(eq2(recipes.id, input.recipeId));
      return { success: true };
    })
  }),
  // Calendar
  calendar: router({
    ...
    addEvent: protectedProcedure.input(z2.object({
      title: z2.string(),
      description: z2.string().optional(),
      type: z2.enum(["savings_goal", "milestone", "couple_event", "task", "reminder"]),
      startDate: z2.date(),
      endDate: z2.date().optional(),
      notifyBefore: z2.number().optional()
    })).mutation(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db) throw new Error("Database not available");
      const couple = await getCoupleByUserId(ctx.user.id);
  ...
  receipts: router({
    ...
    addReceipt: protectedProcedure.input(z2.object({
      store: z2.string(),
      photoUrl: z2.string(),
      photoKey: z2.string(),
      totalAmount: z2.number().optional(),
      date: z2.date(),
      items: z2.array(z2.object({ name: z2.string(), price: z2.number() })).optional()
    })).mutation(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db) throw new Error("Database not available");
      const couple = await getCoupleByUserId(ctx.user.id);
